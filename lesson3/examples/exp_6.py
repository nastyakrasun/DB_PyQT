"""
ORM с помощью SQLAlchemy.
ВАРИАНТ 1: ТРАДИЦИОННЫЙ СТИЛЬ"

ВНИМАНИЕ! SQLAlchemy требует предварительной установки:
...
if __name__ == '__main__':
    ...
    # создание любого пользователя
    # user = User("Толян", "Петров", "pass_Tolyan")
    # sess.add(user)
    # sess.commit()
    print('id: ', user.id)
    print('name: ', user.name)

# # --------------------------чтение в терминале (самодельная наглядность)------------------------- #
# db_full_path = os.path.join(os.path.dirname(__file__), "traditional_style_base.db3")
# # Создание соединения с базой данных
# connection = sqlite3.connect(db_full_path)
# # Создаем курсор
# crs = connection.cursor()
# # --------------------------Подстановка значений в запрос------------------------- #
# crs.execute("SELECT Name FROM users ORDER BY Name LIMIT ?", '3')
# result = crs.fetchall()
