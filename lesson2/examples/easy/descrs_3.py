...
class NonNegative:
    ...
    def __init__(self, my_attr):  # обязательно нужен для инициализации экземпляра класса
        # вместо my_attr на самом деле будет hours или rate
        # например, hours и rate
        self.my_attr = my_attr

    def __get__(self, instance, owner):  # нужен для извлечения значений готовых данных
        ...
        # return abs(instance.__dict__[self.my_attr])  # чтобы значение всегда было неотрицательным
    ...
    def __delete__(self, instance):  # управляем возможностью удаления (можем запретить удалять)
